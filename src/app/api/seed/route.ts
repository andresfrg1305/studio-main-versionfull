 ...
 */
const SEED_KEY = process.env.SEED_DEV_KEY || "CAMBIA_ESTE_VALOR";

export async function GET(req: Request) {
  try {
    if (!db) return NextResponse.json({ ok: false, error: "DB no inicializada" }, { status: 500 });
    if (!authAdmin) return NextResponse.json({ ok: false, error: "Auth Admin no inicializado" }, { status: 500 });
    ...
    const ADMIN_EMAIL = "adminmz5@gestionaph.com";
    const ADMIN_NAME = "Administrador General";
    const ADMIN_PASSWORD = "Admin123!"; // Contraseña temporal
    // ***************************************
    ...
    const ADMIN_UID = adminUser.uid;

    const batch = db.batch();

    // 2) Asegurar perfil admin
    ...
    return NextResponse.json({
      ok: true,
      message: `Seed OK: admin creado con email ${ADMIN_EMAIL} y contraseña ${ADMIN_PASSWORD}, 20 parqueaderos y 1 proyecto creados.`,
    });
  } catch (e: any) {
    console.error(e);
